...
export default async function Overview() {
  ...
  if (!overview) {
    return (
      <div className="panel p-8 text-center" style={{ color: "var(--muted-foreground)" }}>
          ...
          <div className="panel divide-y" style={{ borderColor: "var(--border)" }}>
            {alerts.map((a) => (
              <div key={a.id} className="px-4 py-2.5 flex items-center gap-3 text-sm">
                ...
                <span style={{ color: "var(--foreground)" }}>{a.note}</span>
                <span className="ml-auto mono text-xs" style={{ color: "var(--muted-foreground)" }}>
